        ...
        <main className="flex-1 px-6 py-6">
          <div className="mb-6 flex items-center justify-between gap-4">
            <div className="flex flex-1 items-center gap-3 rounded-2xl border border-slate-200 bg-white/70 px-4 py-3 shadow-sm">
              ...
              />
              <button className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-600">
                Filters
              </button>
            </div>

            <div className="flex items-center gap-3">
              <button type="button" className="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600">
              ...
              <div className="relative">
                ...
                {profileOpen && (
                  <div className="absolute right-0 top-[calc(100%+12px)] z-20 w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(17,44,100,0.18)]">
                    <div className="flex items-start justify-between bg-[#0e2a5c] p-5 text-white">
                      ...
                      <button type="button" onClick={() => setProfileOpen(false)} aria-label="Close profile information" className="rounded-full p-1 text-blue-100 hover:bg-white/10 hover:text-white">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-[1.8fr_0.95fr] gap-6">
            <div className="space-y-6">
              <section className="rounded-[28px] bg-[#eef2f7] p-5 shadow-inner">
                ...
                {!accountStatusLoading && !accountStatusError && profile && settingCard && (
                  <div className="grid gap-3 md:grid-cols-2">
                    {[
                      ...
                      <div key={status.label} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4">
                        <div className="flex items-center gap-3">
                        ...
                        {status.label === "KYC Basic" && !status.active && (
                          <button type="button" className="rounded-xl bg-[#0d2c5f] px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#153d7e]">
                            Verify Identity
                          </button>
                        )}
                      </div>
                  ...
                  <div key={item.title} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-2xl font-semibold text-slate-800">{formatCurrency(item.value)}</p>
                        <p className={`text-xs ${item.tone === "green" ? "text-emerald-500" : item.tone === "red" ? "text-red-500" : "text-sky-500"}`}>
                          {item.change}
                        </p>
                      </div>
                    </div>
              ...
              <div className="grid gap-5 md:grid-cols-[1.3fr_0.8fr]">
                <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <button className="rounded-lg bg-slate-100 px-3 py-1 text-xs text-slate-600">By Day</button>
                  </div>

                  <div className="flex h-[220px] items-end gap-2">
                    {[35, 52, 40, 68, 74, 64, 90].map((height, idx) => (
                      <div key={idx} className="flex flex-1 flex-col items-center gap-3">
                        <div className="w-full rounded-t-xl bg-blue-100" style={{ height: `${height}px` }} />
                ...
                <section className="rounded-[26px] border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={stock.name} className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
                        <div className="flex items-center gap-3">
                  ...
                  <button className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-sky-200 bg-sky-50 px-4 py-3 text-sm font-medium text-sky-700">
            ...
            <aside className="rounded-[28px] bg-[#f0f4f9] p-5 shadow-sm">
              <div className="mb-5 flex items-center justify-between rounded-2xl bg-white p-3 shadow-sm">
                ...
                <div className="relative">
                  ...
                  {settingCardOpen && (
                    <div className="absolute right-0 top-[calc(100%+12px)] z-20 w-[min(320px,calc(100vw-2rem))] overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_20px_50px_rgba(17,44,100,0.18)]">
                      <div className="flex items-start justify-between bg-[#0e2a5c] p-5 text-white">
                        <div className="flex items-center gap-3">
                        ...
                        <button type="button" onClick={() => setSettingCardOpen(false)} aria-label="Close card information" className="rounded-full p-1 text-blue-100 hover:bg-white/10 hover:text-white">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
              ...
              <div className="rounded-[26px] bg-[#0d2c5f] p-5 text-white">
                <div className="mb-6 flex items-center justify-between text-xs text-blue-100">
                  <span>Card Primary</span>
                  <span>•••• 9090</span>
                </div>
                <div className="mb-6 text-4xl font-semibold">{formatCurrency(balance)}</div>
